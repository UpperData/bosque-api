    ...
    .then(async function(rsLotArticle){
        ...
        if(rsLotArticle){ //si el artículo del lote está activo
            if(rsLotArticle['article.isSUW']){ //si se vende por unidad
                ...
                await model.itemLot.findOne({where:{lotId}})
                .then(async function(rsItem){
                    if(!rsItem){ // si es in lote sin Items
                        newItem()//Registrar Item
                        res.status(200).json({data:{"result":true,"message":"Item registrado"}});
                    }else{ // si es un lote con Items
                        res.status(200).json({data:{"result":false,"message":"Solo puede tener un Item para artículo a granel"}});
                    }                    
                })                
            }
        }else{ //si el artículo no esta activo
    ...
    .then(async function(rsAticle){
        if(rsAticle){ // Si el articulo esta activo
            ...
            .then(async function(rsLotValidate){
                ...
                }else{ // Si hay lote activo
                    if(rsAticle.isSUW){ // si la venta es por peso unitario
                        lotEdit(); // registrar lote
                        res.status(200).json({data:{"result":true,"message":"Lote # "+ lotId +"  actualizado"}}); 
                    }else{ // si el producto se vende por peso
                        if(isActived){
        ...
        }else{ // Si articulo esta Inactivo
