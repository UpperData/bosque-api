...
async function getMainCategories(req, res) {
    await model.mainCategory.findAll({ attributes: { exclude: ['isActived', 'createdAt', 'updatedAt'] }, where: { isActived: true } }).then(async function (rsMainCategory) {
...
async function addMainCategories(req, res) {
    ...
    await model.mainCategory.create({ name, icon, url, isActived, order }, { transaction: t }).then(async function (rsMainCategory) {
        t.commit();
...
async function addSubCategories(req, res) {
    ...
    const t = await model.sequelize.transaction();
    await model.subCategory.create({ name, icon, url, isActived, order, mainCategoryId }, { transaction: t }).then(async function (rsResult) {
        t.commit();
...
async function getSubCategoriesN1(req, res) {
    ...
    await model.subCategoryN1.findAll({ attributes: { exclude: ['isActived', 'createdAt', 'updatedAt'] },
        where: { isActived: true, subCategoryId } }).then(async function (rsResult) {
        res.status(200).json({ data: { "result": true, "message": "Consulta satisfactoria", "data": rsResult } });
    }).catch(async function (error) {
        console.log(error);
        res.status(403).json({ data: { "result": false, "message": error.message } });
