...
async function addPermision(req, res) {
    //regsitra un nuevo permiso
    const { subModuleId, operations } = req.body;
    const t = await model.sequelize.transaction();
    try {
        for (let index = 0; index < operations.length; index++) {
            var countPermitions = await model.permission.findAndCountAll({ where: { subModuleId, operationId: operations[index].id } }, { transaction: t }); // valida que no exista el permiso 
            if (countPermitions.count == 0) {
                // si no existe
                await model.permission.create({ subModuleId, operationId: operations[index].id, isActived: operations[index].isActived }, { transaction: t }).then(async function (rsPermissionAdd) {
                    ...
                    for (let index = 0; index < allRoles.length; index++) {
                        await model.permission.findAndCountAll({ where: { roleId: allRoles[index].id, permissionId: rsPermissionAdd.id } }).then(async function (rsFindPermission) {
                            if (rsFindPermission.count == 0) {
                                // si no existe le permiso lo crea
...
async function updatePermission(req, res) {
    ...
    try {
        for (let index = 0; index < operations.length; index++) {
            var countPermition = await model.permission.findAndCountAll({ where: { subModuleId, operationId: operations[index].id } }, { transaction: t }); // valida que no exista el permiso 
            //console.log(countPermition.count);
            if (countPermition.count > 0) {
                // siexiste                
                await model.permission.update({ isActived: operations[index].isActived }, { where: { id: permissionId } }, { transaction: t });
            }
        }
...
async function updatePermissionStatus(req, res) {
    //actualiza estsus de un permiso habilita/deshabilita
    const { permissionId, isActived } = req.body;
    const t = await model.sequelize.transaction();

    await model.permission.update({ isActived }, { where: { id: permissionId } }, { transaction: t }).then(async function (rsPermission) {
        t.commit();
        res.status(200).json({ "data": { "result": true, "message": "Proceso Satisfactorio" } });
    }).catch(async function (error) {
        t.rollback();
