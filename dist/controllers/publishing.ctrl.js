...
async function getPublishing(req, res) {
    ...
    if (articleId != '*') {
        ...
        return await model.inventory.findAll({ where: { articleId, isPublished: true }, order: ['updatedAt'] }).then(async function (rsPublishing) {
            res.status(200).json({ "data": { "result": true, "message": "Busqueda satisfatoria", "data": rsPublishing } });
        }).catch(async function (error) {
            console.log(error);
            res.status(403).json({ "data": { "result": false, "message": "Algo salió mal buscando registro" } });
...
async function setPublishing(req, res) {
    ...
    return await model.article.update({ isPublished }, { where: { id: articleId } }, { transaction: t }).then(async function (rsPublishing) {
        await t.commit();
        if (isPublished) {
            // notifica nuevos productos disponible
...
async function getPublishingSubCategoryAndText(req, res) {
    // busca lo escriba el usuario en una categoria
...
async function getPublishingByShopSeven(req, res) {
    //últimas 7 publicaciones de una tienda  
