...
async function newToken(account, roles, type, dateTime, people, shop) {
	//Genera un token con una estructura especifica -->newToken(account,roles,type,dateTime,people)
	var exp;
...
async function dataTokenGet(token) {
	// obtiene informacion del token con la estructura --> newToken(account,roles,type,dateTime,people,shop)
	try {
...
async function genRestoreSecret(rsAccount) {
	//genera toke sin estructura especifica
	try {
...
async function getTokenAll(token) {
	// obtiene token sin estructura especifica --> payload
	try {
		var payload = await jwt.decode(token, process.env.JWT_SECRET);
		if (Date.now() >= payload.exp * 1000) {
			return false;
