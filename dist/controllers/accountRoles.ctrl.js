...
async function add(req, res) {
	...
	return await models.accountRoles.create({ accountId, RoleId, StatusId: statusId }, { transaction: t }).then(function (rsResult) {
		t.commit();
		return rsResult;
	}).catch(async function (error) {
		t.rollback();
...
async function addMembership(req, res) {
	...
	return await models.accountRole.findAndCountAll({
		...
		if (rsMembership.count > 0) {
			//actualiza
			await models.accountRole.update({ isActived }, { where: { accountId, roleId }, transaction: t }).then(function (rsResult) {
				t.commit();
				if (isActived) {
			...
			//crea
			await models.accountRole.create({ accountId, roleId, isActived }, { transaction: t }).then(function (rsResult) {
				t.commit();
