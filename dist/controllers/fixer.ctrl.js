...
async function fixerPermissions(req, res) {
    ...
    await model.role.findAll().then(async function (rsRole) {
        for (let index = 0; index < rsRole.length; index++) {
            // recorre todos lo roles   
            let t = await model.sequelize.transaction();
            if (rsRole[index].id != 5) {
                await model.permission.findAll().then(async function (rsPermission) {
                    //recorre permisos 
                    for (let j = 0; j < rsPermission.length; j++) {
                        try {
                            ...
                            await model.grantRole.update({ roleId: rsRole[index].id, permissionId: rsPermission[j].id, isActived: false }, { where: { roleId: rsRole[index].id, permissionId: rsPermission[j].id } }, { transaction: t });
                            updated++;
