...
async function voucherGetById(req, res) {
    ...
    } else if (employeeFileId > 0 && voucherId == "null") {
...
async function voucherDisable(req, res) {
    //Anular recibo
    const { voucherId } = req.params;
    const t = model.sequelize.transaction();
    return await model.voucher.update({ isActived: false }, { where: { id: voucherId } }, { transaction: t }).then(async function (rsVoucher) {
