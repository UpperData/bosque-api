...
async function createStore(req, res) {
    ...
    await model.store.create({ name, logo, description, isItHaveBuild, phone, address,
        parroquiaId, storeTypeId, isActived, fiscalInfo, deliveryInfo, accountId }, { transaction: t }).then(async function (rsStoreCreate) {
        t.commit();
...
async function updateStore(req, res) {
    ...
    await model.store.update({ name, logo, description, isItHaveBuild, phone, address,
        parroquiaId, storeTypeId, isActived, fiscalInfo, deliveryInfo }, { where: { id } }, { transaction: t }).then(async function (rsStoreCreate) {
        t.commit();
...
async function createContract(req, res) {
    ...
    const t = await model.sequelize.transaction();
    await model.contract.findAndCountAll({ where: { storeId, isActived: true } }).then(async function (rsFindContract) {
        if (rsFindContract.count > 0) {
            ...
            await model.contract.create({ startDate, endDate, comission, storeId, isActived, fileContract }, { transaction: t }).then(async function (rsResult) {
                t.commit();
...
async function revokeContract(req, res) {
    ...
    const t = await model.sequelize.transaction();
    await model.contract.update({ isActived }, { where: { id } }, { transaction: t }).then(async function (rsContract) {
        t.commit();
...
async function contractFile(req, res) {
    ...
    const t = await model.sequelize.transaction();
    await model.contract.update({ fileContract }, { where: { id } }, { transaction: t }).then(async function (rsContract) {
        t.commit();
        res.status(200).json({ "data": { "result": true, "message": "Contrato actualizado satisfactoriamente" } });
    }).catch(async function (error) {
        t.rollback();
...
async function findContractByStore(req, res) {
    ...
    return await model.contract.findAll({ where: { storeId } }, { order: ['createdAt'] }).then(async function (rsContract) {
        res.status(200).json({ "data": { "result": true, "message": "Busqueda satisfatoria", "data": rsContract } });
    }).catch(async function (errror) {
        console.log(errror);
        res.status(403).json({ "data": { "result": false, "message": "Algo salió mal buscando registro" } });
