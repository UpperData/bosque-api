...
async function registerAccount(req, res) {
    ...
    //pass = pass.slice(-6);
    let pass = people.document.numbre.toString();
    //genera username
    let name;
    ...
    await model.account.create({ email, photo, phone, name, pass, people, creater: audit, secret: null, token: "null", isConfimr: true, isActived: true }, { transaction: t }).then(async function (rsAccount) {
    ...
    return await model.account.findOne({
        ...
        if (rsUser) {
            ...
            if (!rsUser.isActived) {
                ...
                if (rsUser.tries > 5) {
                    await model.account.update({ isActived: false, hashConfirm: null }, { where: { id: rsUser.id } }).then(async function (rsStatus) {
                        //Actualiza status                               
                        ...
                        await model.notification.create({
                            ...
                            body: {
                                ...
                                link: process.env.COMPANY + "/account/details/" + rsUser.id
                            },
                            read: false,
                    ...
                    /* return await  bcrypt.compare(pass,rsUser.pass)//////
                     .then(async  function (rsPass){////////
                         if(rsPass){	//////		*/
                    return await roleAccount.getRoleByAccount({ accountId: rsUser.id }).then(async function (rsAccRoles) {
                        if (rsAccRoles.length > 0) {
                            ...
                            await model.account.update({ tries: 0 }, { where: { id: rsUser.id } }).then(async function (rsNewPassword) {
                                //Actualiza tries
...
async function passwordRestart(req, res) {
    ...
    .then(async function (rsAccount) {
        ...
        if (rsAccount) {
            //Valida que no pase de 5 intentos
            if (rsAccount.tries < 6) {
                //Valida que no pase de 5 intentos
                if (rsAccount.hashConfirm === token) {
                    ...
                    if (answerValid) {
                        //valida respuesta secreta es correcto
                        await model.account.update({ pass: newPassword, hashConfirm: null, tries: 0 }, { where: { id: rsAccount.id } }).then(async function (rsNewPassword) {
                            //Actualiza password
                        ...
                        //valida respuesta secreta es invalida
...
async function passwordUpdate(req, res) {
    ...
    await model.account.findOne({ attributes: ['pass', 'email'], where: { id: dataToken['account'].id } }).then(async function (rsAccount) {
        await bcrypt.compare(currentPassword, rsAccount.pass).then(async function (rsValid) {
            if (rsValid) {
                return await model.account.update({ pass: newPassword }, { where: { id: dataToken.account.id } }, { transaction: t }).then(async function (rsNewPassword) {
                    //Actualiza password                    
...
async function resetSecretAnswer(req, res) {
    ...
    if (dataToken.payload.exp <= moment().unix()) {
        // Valida expiración                      
        res.redirect(process.env.HOST_FRONT + "idetificationError?message=" + "Su token a expirado, debe generar uno nuevo");
    } else {
        await model.Account.findOne({ where: { id: dataToken.payload['account'].id } }).then(async function (rsAccount) {
            if (!rsAccount.isActived) {
...
async function getProfile(req, res) {
    //optiene el perfil de un usuario
...
async function getRoleByAccount(req, res) {
    // OBTIENE ROLES DE CUENTA ACTUAL
    const token = req.header('Authorization').replace('Bearer ', '');
    const currentAccount = await serviceToken.dataTokenGet(token);
...
async function emailUpdate(req, res) {
    ...
    await model.account.findOne({ attributes: ['pass', 'email'], where: { id: dataToken['account'].id } }).then(async function (rsAccount) {
        ...
        await bcrypt.compare(currentPassword, rsAccount.pass).then(async function (rsValid) {
            // Valida password
            if (rsValid) {
                return await model.account.update({ email: newEmail, isConfirmed: false }, { where: { id: dataToken['account'].id } }, { transaction: t }).then(async function (rsNewEmail) {
                    //Actualiza email                    
...
async function emailCertificationToken(req, res) {
    //Genera y envia  token para certificacion de email
    //optiene email
    const dataToken = await serviceToken.dataTokenGet(req.header('Authorization').replace('Bearer ', ''));
    if (dataToken) {
        //Genera token
...
async function isCertificated(req, res) {
    // Verifica si un email esta certificado    
    const dataToken = await serviceToken.dataTokenGet(req.header('Authorization').replace('Bearer ', ''));
    if (dataToken) {
...
async function getSecretCurrent(req, res) {
    // obtiene preguntas secretas del sesion actual
    const dataToken = await serviceToken.dataTokenGet(req.header('Authorization').replace('Bearer ', ''));
    if (dataToken) {
        return await model.account.findOne({
            attributes: ['secret', 'isActived'],
...
async function getActiveAccount(req, res) {
    return await model.account.findAll({ attributes: ['id', 'email'], where: { isActived: true } }).then(async function (rsAccount) {
...
async function buyerRegisterAccount(req, res) {
    ...
    // OPtiene todos los administradores del sistema (email) 
    await model.account.create({ email, name, pass, people, creater: audit, secret: null, token: "null", isConfirmed: true, isActived: true }, { transaction: t }).then(async function (rsAccount) {
...
async function validatePhone(req, res) {
    ...
    return await model.account.findOne({
        ...
        if (rsAccount) {
            if (rsAccount.isActived) {
                res.status(200).json({ data: { "result": true, "message": "Número de teléfono valido", "data": rsAccount.phone } });
            } else {
                res.status(403).json({ data: { "result": false, "message": "Cuenta bloqueada comuniquese con el administrador del sistema" } });
            }
        } else {
            res.status(403).json({ data: { "result": false, "message": "Número de teléfono no es valido" } });
        }
    }).catch(async function (error) {
        console.log(error);
        res.status(403).json({ data: { "result": false, "message": "Algo salió mal validando Número de teléfono" } });
    });
}
module.exports = {
    registerAccount,
