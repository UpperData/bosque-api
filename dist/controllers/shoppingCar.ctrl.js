...
async function cancelShoppincar(req, res) {
    ...
    console.log(itemLot.shoppingCarId);
    return await model.shoppingCar.update({ orderStatusId: 6 }, { where: { id: itemLot.shoppingCarId }, returning: true, transaction: t }).then(async function (rsUpdateShpp) {
        /*  var qtyCar=rsUpdateShpp.qty||0;            
         var qtyItem=itemLot.weight||0;  
         var qty=qtyItem+qtyCar; */
        // activar lote
        await model.lots.update({ isActived: true }, { where: { id: itemLot.lotId }, transaction: t }).then(async function (rsActiveLot) {
            await model.itemLot.update({ conditionId: 1 /* ,weight:qty */ }, { where: { id: itemLot.id }, transaction: t }).then(async function (rsUpdateItem) {
                t.commit();
...
async function editShoppincar(req, res) {
    ...
    return await model.shoppingCar.update({ dispatch, isActived }, { where: { id }, transaction: t }).then(async function (rsUpdate) {
        await model.itemLot.findOne({ attributes: ['weight'], where: { id: itemLotId }, transaction: t }).then(async function (rsExistence) {
            ...
            if (rsExistence.weight >= parseFloat(dispatch)) {
                ...
                await model.itemLot.update({ weight: diff, isActived }, { where: { id: itemLotId }, transaction: t });
            } else {
                t.rollback();
            ...
            for (let Jindex = 0; Jindex < rsCar[index]['lots'].length; Jindex++) {
                ...
                if (rsCar[index]['lots'].length > 0) {
                    for (let Kindex = 0; Kindex < rsCar[index]['lots'][Jindex]['itemLots'].length; Kindex++) {
                        ...
                        for (let Mindex = 0; Mindex < rsCar[index]['lots'][Jindex]['itemLots'][Kindex]['shoppingCars'].length; Mindex++) {
                            // nivel shoppingCar
                            if (rsCar[index].isSUW) {
...
async function AddShoppingCar(req, res) {
    ...
    return await model.itemLot.findAndCountAll({ attributes: ['id'], where: { id: itemLotId, conditionId: 1 } }).then(async function (rsItem) {
        //console.log(rsItem.dataValues);
        if (rsItem.count > 0) {
            // Item disponible            
            //valida cuenta activa
            await model.account.findAndCountAll({ attributes: ['id'], where: { id: accountId, isActived: true } }).then(async function (rsAccount) {
                let rsCondition;
                if (rsAccount.count > 0) {
                    ...
                    if (itemActived.length > 0) {
                        // si item esta disponible
                        if (SUW) {
                            // venta por unidapesada
                            newQty = 0;
                        ...
                        await model.shoppingCar.create({ itemLotId, accountId, dispatch, orderStatusId: 1, newQty, audit }, { transaction: t }).then(async function (rsCar) {
                            t.commit();
