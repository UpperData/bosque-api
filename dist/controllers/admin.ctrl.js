...
async function passwordResetEnable(req, res) {
    // hablita el cambio de password
    ...
    return await model.account.findOne({
        ...
        if (rsUser) {
            ...
            await model.account.update({ hashConfirm: token }, { where: { id: rsUser.id } }, { transaction: t }).then(async function (rsAccount) {
                await t.commit();
...
async function activateAccount(req, res) {
    const { email } = req.params;
    await model.account.update({ isActived: true, tries: 6 }, { where: { email } }).then(async function (rsAcitvteAccount) {
        if (await utils.isInternetConnect()) {
...
async function passwordResetRevoke(req, res) {
    // hablita el cambio de password
    const { id } = req.params;
    const t = await model.sequelize.transaction();
    return await model.account.findOne({
        ...
        if (rsUser) {
            await model.account.update({ hashConfirm: null }, { where: { id: rsUser.id } }, { transaction: t }).then(async function (rsAccount) {
                await t.commit();
