...
async function inventoryArticle(req, res) {
    // optiene el inventario actual, hoja de inventario
            ...
            await model.inventory.findOne({
                ...
                if (rsExistence) {
                    if (rsExistence.existence >= quantity) {
                        await model.assignment.create({ accountId: accountId, articleId, quantity, responsible: { "account": dataToken['account'], "people": dataToken['people'] } }, { transaction: t }).then(async function (rsAssignmet) {}).then(async function (rsAppointment) {
                            t.commit();
    ...
    await model.accountRole.findAndCountAll({
        ...
        if (rsAccountRole.count > 0) {
            await model.inventory.findOne({
                ...
                if (rsExistence) {
                    if (rsExistence.existence >= quantity) {
                        await model.assignment.update({ accountId: accountId, articleId, quantity, responsible: { "account": dataToken['account'], "people": dataToken['people'] } }, { where: { id: assignmentId } }, { transaction: t }).then(async function (rsAssignmet) {}).then(async function (rsAppointment) {
                            t.commit();
                            res.status(200).json({ data: { "result": true, "message": "Asignación satisfactoria" } });
                        }).catch(async function (error) {
                            console.log(error);
...
async function articleNew(req, res) {
    ...
    await model.article.create({ id: maxArticle[0][0].proximo, name, description, minStock, image, price, isSUW, audit }, { transaction: t }, { returning: true }).then(async function (rsArticle) {
        t.commit();
...
async function inventoryAdd(req, res) {
    ...
    await model.inventory.findAndCountAll({ articleId, isActived: true }).then(async function (rsFind) {

        if (rsFind.count > 0) {// inventory update

        } else {
            await model.inventory.create({ articleId, existence, minStock, price, category, sku, filter, description, tags, photo }).then(async function (rsInventoryCreate) {});
        }
    });
}
async function inventoryTotal(req, res) {
    // optiene el inventario actual, hoja de inventario
    const dataToken = await generals.currentAccount(req.header('Authorization').replace('Bearer ', ''));
    await model.article.findAll({
        ...
        for (let index = 0; index < rsInventory.length; index++) {
            ...
            asignados = await model.assignment.findOne({
                attributes: [[model.sequelize.fn('sum', model.sequelize.col('quantity')), 'total_asignament']],
            ...
            totalPriceInventory = totalPriceInventory + parseFloat(rsInventory[index].price).toFixed(2) * parseFloat(rsInventory[index].dataValues.almacen == '-' ? 0 : rsInventory[index].dataValues.almacen).toFixed(2);
...
async function inventoryUpdate(req, res) {
    ...
    await model.assignment.findOne({
        ...
        if (rsAssinament.total_amount < existence) {
            // La existencia no debe ser menos a lo que esta en asignación
...
async function returnArticleArray(req, res) {
    ...
    if (!Array.isArray(articleList.qs)) {
        // cuando pasa un solo elemento (no es un arreglo) lo convierte en arreglo
