...
async function medicalRepostNew(req, res) {
    ...
    await model.medicalReport.create({ appointmentId, description, withExams, withMedicine, medicines, exams, dosage, otherExams }, { tranasction: t }).then(async function (rsReport) {
        await model.appointment.update({ isOpened: false }, { where: { id: appointmentId } }, { tranasction: t }).then(async function (rsAppointment) {
            t.commit();
...
async function medicalReportEdit(req, res) {
    ...
    const t = await model.sequelize.transaction();
    await model.medicalReport.update({ appointmentId, description, withExams, withMedicine, medicines, exams, dosage, otherExams }, { where: { id } }, { tranasction: t }).then(async function (rsReport) {
        t.commit();
